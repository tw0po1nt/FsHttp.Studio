...
/// they drop the parse-failed flag here rather than at every call site. The tests that assert
/// over that flag call `locateBlocks` directly.
...
let private locate (source: string) : BlockRange list =
    blocksOf source |> List.map (fun lb -> lb.Block)
