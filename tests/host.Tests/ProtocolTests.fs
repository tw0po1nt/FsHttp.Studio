...
let parseFailedOrDefaultTests =
    testList
        "parseFailedOrDefault"
        [ test "an absent value decodes to false" {
              Expect.isFalse (parseFailedOrDefault None) "an old companion that omits the property"
          }

          test "a present false decodes to false" {
              Expect.isFalse (parseFailedOrDefault (Some false)) "a clean source"
          }
